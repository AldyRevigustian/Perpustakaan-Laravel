{{-- Title --}}<title>
    {{ __('Registruj se | Online biblioteka') }}</title>{{-- Icon --}}
<link rel="icon" type="image/x-icon" href="{{ asset('library-favicon.ico') }}">{{-- Style --}}
<link rel="stylesheet" href="{{ asset('css/custom-style/style.css') }}">
<!DOCTYPE html>
<html lang="en">
        ...
        <div class="flex items-center justify-center pt-[13%]">\
            <div class="w-full max-w-md">
                <form class="px-12 pt-6 pb-4 mb-4 bg-white rounded shadow-lg" method="POST"
                    action="{{ route('register') }}">@csrf @honeypot
                    <div class="flex justify-center py-2 mb-4 text-2xl text-gray-800 border-b-2">
                        Online biblioteka <img height="5px" width="34px" class="ml-2"
                            src="{{ asset('library-favicon.ico') }}" alt="Online biblioteka" title="Online biblioteka">
                    </div>
                    <div class="mb-4"><label for="name"
                            class="block mt-2 mb-2 text-sm font-normal text-gray-700">Ime
                            i prezime &nbsp; @error('name')
                        ...
                        <input
                            ...
                            id="name" name="name" type="text" required autofocus value="{{ old('name') }}">
                    </div>
                    <div class="mb-4"><label for="email"
                            class="block mt-2 mb-2 text-sm font-normal text-gray-700">Email
                            adresa &nbsp; @error('email')
                                <span style="color:#cd1a2b;font-size:15px">/ {{ $message }}<i
                                        class="fa-solid fa-exclamation fa-lg"></i></span>
                            @enderror
                        </label>
                        <input
                    ...
                    <div class="mb-4"><label for="JMBG"
                        ...
                        <input
                            ...
                            id="JMBG" name="JMBG" type="number" required autofocus
                            value="{{ old('JMBG') }}">
                    </div>
                    ...
                    <div class="mb-6"><label class="block mb-2 text-sm font-normal text-gray-700"
                            ...
                            v-model="form.password" type="password" name="password" required id="password"
                    ...
                    <div class="mb-6"><label class="block mb-2 text-sm font-normal text-gray-700"
                            ...
                            v-model="form.password" type="password" name="password_confirmation" required
                            id="password_confirmation" autocomplete="current-password"></div>
                    <div class="flex items-center justify-between">
                        ...
                        @if (Route::has('password.request'))
                            <a class="inline-block text-sm font-normal text-black align-baseline transition-custom hover:text-blue-800"
                                href="{{ route('login') }}">Imate nalog?</a>
                        @endif
                    </div>
